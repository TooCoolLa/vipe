...
# -------------------------------------------------------------------------------------------------
# This file provides an adapter to wrap Pi3X model from the Pi3 repository
# (https://github.com/yyfz/Pi3) to match the DroidNet interface used in VIPE.
# -------------------------------------------------------------------------------------------------
...
class DroidNet(nn.Module):
    """
    DroidNet adapter that wraps Pi3X model.
    
    This class provides the same interface as the original DroidNet while internally
    using the Pi3X model from the Pi3 repository. The input/output signatures are
    preserved for backward compatibility with the existing VIPE codebase.
    """
    ...
    def load_weights(self):
        ...
        # Try to load from local ckpt directory first - check for Pi3X checkpoint
        local_pi3x_path = Path("./ckpt/slam/pi3x/pi3x.pth")
        local_ckpt_path = Path("./ckpt/slam/droid/droid.pth")
        
        if local_pi3x_path.exists():
            print(f"Loading Pi3X-compatible model from local path: {local_pi3x_path}")
            ckpt_path = local_pi3x_path
